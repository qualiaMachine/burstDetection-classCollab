...
public class Lab3_Endemann_Sescleifer_Wolfe {
	...
	public static void main(String[] args) {
		...
		if (args.length >= 4) {  imageLength     = Integer.parseInt(args[3]); }
		if (args.length >= 5) {  imageWidth = Integer.parseInt(args[4]);}
	...
	private static Vector<Example> convertExamples(Dataset dataset) {
		...
		for (Instance image : images) {
			...
			double[][][] features = new double[imageLength][imageWidth][unitsPerPixel];
...
//			this.doneTraining = false; // necessary for implementing dropout
		...
		this.layers.add(new FullyConnectedLayer(5, 5, 16, 128,dropOut,layers.get(3)));// **may want to test out varying #nodes in fully connected layer
		this.layers.add(new FullyConnectedLayer(128, 1, 1, 6,dropOut,layers.get(4)));
		this.contextlayers.add(new ConvolutionLayer(32, Lab3_Endemann_Sescleifer_Wolfe.unitsPerPixel, 8, 5,dropOut,normalizeKernelOutputByKernelSum));
		this.contextlayers.add(new PoolLayer(28, 8,2,dropOut));// ***may want to test out different numFilters
		this.contextlayers.add(new ConvolutionLayer(14, 8, 16, 5,dropOut,normalizeKernelOutputByKernelSum));
		this.contextlayers.add(new PoolLayer(10, 16,2,dropOut));
		this.contextlayers.add(new FullyConnectedLayer(5, 5, 16, 128,dropOut,layers.get(3)));// **may want to test out varying #nodes in fully connected layer
		this.contextlayers.add(new FullyConnectedLayer(128, 1, 1, 6,dropOut,layers.get(4)));
		this.doneTraining = false; // necessary for implementing dropout
...
//	        System.out.println(image.length+" "+image[1].length+" "+image[0][1].length);
	...
	public void train(double[][][] image, int label) {
		...
		double[][][] contexterrors = new double[6][1][1];
		for (int i = 0; i < 2; i++) {
	...
	public ConvolutionLayer(int inputLength, int inputWidth, int inputDepth, int numFilters, int filterSize, boolean dropOutLayer, boolean normalizeKernelOutputByKernelSum) {
		...
		for (int x = 0; x < filterSize; x++) {
			for (int y = 0; y < filterSize; y++) {
				for (int z = 0; z < inputDepth; z++) {
					for (int filter = 0; filter < numFilters; filter++) {
						this.weights[x][y][z][filter] = Lab3_Endemann_Sescleifer_Wolfe.getRandomWeight(inputLength * inputWidth * inputDepth, outputLength * outputWidth * numFilters);
					}
				}
			}
		}
...
//											}
			...
			for (int x = 0; x < this.outputLength; x++) {
				for (int y = 0; y < this.outputWidth; y++) {
					for (int filter = 0; filter < this.numFilters; filter++) {
						double error = errors[x][y][filter];
						this.biases[filter] -= Lab3_Endemann_Sescleifer_Wolfe.eta * error * (1.0 / (this.outputLength * this.outputWidth)); // Update bias
						this.updateWeights(error * (1.0 / (this.filterSize * this.filterSize)), x, y, filter);
					}
				}
			}
