...
private static final int    maxEpochs = 25; // Feel free to set to a different value.
...
private static int trainDeep(Vector<Example> train, Vector<Example> tune, Vector<Example> test) {
	...
	try (
			BufferedWriter testwriter = new BufferedWriter(new FileWriter("testerror.txt"))
			;BufferedWriter tunewriter = new BufferedWriter(new FileWriter("tuneerror.txt"))
			;BufferedWriter trainwriter = new BufferedWriter(new FileWriter("trainerror.txt"))
	    ){
		for (int epoch = 1; epoch <= maxEpochs /* && trainSetErrors > 0 */; epoch++) { // Might still want to train after trainset error = 0 since we want to get all predictions on the 'right side of zero' (whereas errors defined wrt HIGHEST output).
			...
			testwriter.write(comma((double) testSetError / test.size()));
			testwriter.newLine();
			trainwriter.write(comma((double) trainSetError / train.size()));
			trainwriter.newLine();
			tunewriter.write(comma((double) tuneSetError / tune.size()));
			tunewriter.newLine();
